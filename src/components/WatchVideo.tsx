...
import VideoCards, { VIEW_FORMATER } from "./VideoCards";
import {
  BiArrowFromBottom,
  BiArrowFromTop,
  BiCut,
...
const WatchVideo = () => {
  ...
  return (
    <div className="flex">
      <div className="h-full w-[1350px] flex flex-col px-6 py-4">
        {video ? (
      ...
      <div className="max-w-[500px] pt-5">
        <div
          className={`w-full bg-blue-400 ${
            !isOpen ? "h-[80px]" : "h-[800px] flex-col "
          } rounded-lg flex gap-10 p-5`}
        >
          <div className="flex items-center justify-between w-full">
            <span className={`text-2xl font-sans`}>Playlist Video</span>{" "}
            <Button
              variant={"default"}
              size={"icon"}
            ...
            >
              {!isOpen ? (
                <BiArrowFromTop className="text-2xl" />
              ) : (
                <BiArrowFromBottom className="text-2xl" />
              )}
            </Button>
          </div>
          <div className="flex flex-col overflow-x-hidden gap-4 top-4">
            {isOpen &&
              data.map((items) => (
                <div className="w-full">
                  <VideoCards
                    cardType="Playlist"
                    className="flex h-[99px] w-full"
                    key={items.id}
        ...
        <div className="sticky top-0 bg-white z-10 pb-4 mt-4">
          <Category
            type="Watch"
